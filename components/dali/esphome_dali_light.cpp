...
void dali::DaliLight::setup_state(light::LightState *state) {
    ...
    if ((this->address_ != ADDR_BROADCAST) && ((this->address_ & ADDR_GROUP_MASK) == 0)) {
        ...
        if (bus->dali.isDevicePresent(address_)) {
            ...
            if (this->dali_level_min_>161) {
                ESP_LOGD(TAG, "Forced minLevel %d to 161", this->dali_level_min_);
                this->dali_level_min_=161;
            }
            if (this->dali_level_max_==255) {
                ESP_LOGD(TAG, "Forced maxLevel %d to 254", this->dali_level_max_);
